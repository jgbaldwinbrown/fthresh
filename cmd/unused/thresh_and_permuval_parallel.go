...
func ThreshMergeAndPerm(set PlotSet, seed int) (err error) {
	...
	bedpaths_path := "bedpaths_scripted_" + set.Out + ".txt"
...
func TMPParallel(set PlotSet, seed int, errchan chan error) {
	err := ThreshMergeAndPerm(set, seed)
	errchan <- err
}

func TMPSets(sets PlotSets) (errors []error) {
	var errorchans []chan error
	for seed, set := range sets {
		ec := make(chan error)
		errorchans = append(errorchans, ec)
		go TMPParallel(set, seed, ec)
	}
